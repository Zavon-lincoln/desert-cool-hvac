...
import { LogOut, Calendar, Users, Lock, ChevronLeft, ChevronRight, CheckCircle, Clock, XCircle, Phone, Mail } from 'lucide-react'
      ...
      <div className="min-h-screen bg-brand-dark flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-gray-500 text-sm mt-1">Desert Cool HVAC Dashboard</p>
          </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-brand-dark shadow-lg sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-bold leading-tight">Desert Cool HVAC</p>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'table' ? 'bg-brand-orange text-white' : 'text-gray-300 hover:text-white'}`}
            >
              <Users className="w-4 h-4" /> Leads
            </button>
            <button
              onClick={() => setView('calendar')}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'calendar' ? 'bg-brand-orange text-white' : 'text-gray-300 hover:text-white'}`}
            >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Leads',  value: stats.total,     color: 'text-brand-orange', bg: 'bg-orange-50' },
                    ...
                    {leads.map(lead => (
                      <tr key={lead.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-gray-600">
                          <a href={`tel:${lead.phone}`} className="flex items-center gap-1 hover:text-brand-orange">
                            <Phone className="w-3 h-3" />{lead.phone}
                          </a>
                        </td>
                        <td className="px-4 py-3 text-gray-600">
                          <a href={`mailto:${lead.email}`} className="flex items-center gap-1 hover:text-brand-orange">
                        ...
                        <td className="px-4 py-3">
                          <select
                            ...
                            className={`text-xs font-semibold px-2 py-1 rounded-full border-0 cursor-pointer focus:outline-none focus:ring-2 focus:ring-brand-orange ${STATUS_STYLES[lead.status]}`}
                          >
            ...
            <div className="grid grid-cols-7">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <div
                    key={day}
                    className={`min-h-[80px] border-b border-r border-gray-100 p-2 ${isToday ? 'bg-orange-50' : 'hover:bg-gray-50'} transition-colors`}
                  >
                    <span className={`text-sm font-semibold inline-flex items-center justify-center w-7 h-7 rounded-full ${isToday ? 'bg-brand-orange text-white' : 'text-gray-700'}`}>
                      {day}
                    </span>
                    <div className="mt-1 space-y-1">
                      {appts.slice(0, 2).map(a => (
                        <div key={a.id} className="text-xs bg-brand-orange/10 text-brand-orange font-medium rounded px-1.5 py-0.5 truncate">
                          {a.name}
                        </div>
                      ))}
                      {appts.length > 2 && (
            ...
            {Object.keys(appointmentsByDay).length > 0 && (
              <div className="px-6 py-4 border-t border-gray-100">
                <p className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
                  <Clock className="w-4 h-4 text-brand-orange" /> Appointments this month
                </p>
